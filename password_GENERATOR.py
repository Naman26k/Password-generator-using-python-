...
def low():
	...
	length = var1.get()
	lowerp = "abcdefghijklmnopqrstuvwxyz"
	upperp = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
	digitsp = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 !@#$%^&*()"
	password = ""
	...
	if var.get() == 0:
		for i in range(0, length):
			password = password + random.choice(lowerp)
		return password
	elif var.get() == 1:
		for i in range(0, length):
			password = password + random.choice(upperp)
		return password

	elif var.get() == 3:
		for i in range(0, length):
			password = password + random.choice(digitsp)
		return password
	else:
